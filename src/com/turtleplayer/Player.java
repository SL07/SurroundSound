...
public class Player extends ListActivity
{
	...
	public enum Slides
	{
		SETTINGS, //Change to surround sound 
		PLAYLIST,
		NOW_PLAYING
	}
	...
	private ImageView settings; //change to surround sound 

	// Slides
	private RelativeLayout nowPlayingSlide;
	private RelativeLayout playlistSlide;
	private RelativeLayout settingsSlide; //change to surround sound 
	...
	// Settings Slide						Change to fit surround sound 
	...
	//Surround Sound Slide
	TextView welcomeText;
	TextView musicAround;
	ToggleButton onAndOff;
	...
	private void lookupViewElements(){
		...
		settings = (ImageView) findViewById(com.turtleplayerv2.R.id.settingsButton); //Change to Surround Sound 
		...
		welcomeText = (TextView) findViewById(com.turtleplayerv2.R.id.welcomeText);
		musicAround = (TextView) findViewById(com.turtleplayerv2.R.id.musicAround);
		musicAround.setMovementMethod(new ScrollingMovementMethod());
		onAndOff = (ToggleButton) findViewById(com.turtleplayerv2.R.id.onAndOff);
	}
	...
	private void SetupButtonListeners()
	{
		...
		logo.setOnClickListener(new OnClickListener() //Change to a button more relevant to now playing 
		{
			public void onClick(View v)
			{
				SwitchToNowPlayingSlide();
		...
		settings.setOnClickListener(new OnClickListener() //Change to Surround Sound 
		{
			public void onClick(View v)
			{
		...
		backButton.setOnClickListener(new OutputUsingOnClickListener(tp.player)
		{
			...
			{
				SurroundSoundUpdate update = new SurroundSoundUpdate(Player.this, output.getCurrTrack(), "remove");
				update.execute("");
				
				output.play(tp.playlist.getPrevious(standartPlayOrderStrategy, output.getCurrTrack()));
				currentTrack = output.getCurrTrack(); 
				
				update = new SurroundSoundUpdate(Player.this,currentTrack, "add");
				update.execute("");
			}
		});
		...
		shuffleButton.setOnClickListener(new OutputUsingOnClickListener(tp.player)
		{
			...
			public void onClick(View v,Output output)
			{
				SurroundSoundUpdate update = new SurroundSoundUpdate(Player.this, output.getCurrTrack(), "remove");
				update.execute("");
				output.play(tp.playlist.getNext(shufflePlayOrderStrategy, output.getCurrTrack())); //Can see here how getCurrTrack() is called 
				currentTrack = output.getCurrTrack();
				update = new SurroundSoundUpdate(Player.this,currentTrack, "add");
				update.execute("");
			}
	...
	private void SetupSlides()
	{
		...
		settingsSlide = (RelativeLayout) findViewById(com.turtleplayerv2.R.id.settings_slide);  //Change to Surround Sound 
	...
	private void SwitchToNowPlayingSlide()
	{
		...
		settingsSlide.setVisibility(LinearLayout.INVISIBLE); //Change to Surround Sound
	}
	...
	private void SwitchToPlaylistSlide()
	{
		...
		//Log.i("Setting Slid", "Current Track: " + currentTrack.getSongName()); 
	}

	private void SwitchToSettingsSlide() //Change to be relevant to Surround Sound 
	{
		ResetHeaderButtons();
		//Make pull from database
		SurroundSoundRetrieve ssRetrieve = new SurroundSoundRetrieve(Player.this, "GetCurrentPlaying");
		ssRetrieve.execute("");
		...
		nowPlayingSlide.setVisibility(LinearLayout.INVISIBLE);
		playlistSlide.setVisibility(LinearLayout.INVISIBLE);
		settingsSlide.setVisibility(LinearLayout.VISIBLE); //Change to surround sound 
		
		//Log.i("SettingsSlide", "Selected Track: " + currentTrack.getSongName());
		...
		if (trackSelected != null)
		{
			...
			tp.player.connectPlayer(new OutputCommand()
			{
				public void connected(Output output)
				{
					SurroundSoundUpdate update = new SurroundSoundUpdate(Player.this, output.getCurrTrack(), "remove");
					update.execute("");
					update = new SurroundSoundUpdate(Player.this, trackSelected, "add");
					update.execute("");
...
// ========================================= //
	...
	private void ResetHeaderButtons()
	{
		...
		settings.setImageDrawable(getResources().getDrawable(com.turtleplayerv2.R.drawable.settings48)); //Change to surround sound 
	}
